...
patch("src.bot.config.settings.bot.token", "valid:test_token_for_testing").start()
...
async def test_start_command(mock_message):
    ...
    assert "Welcome to the Anonymous Scream Bot" in mock_message.reply.call_args[0][0]
...
@pytest.mark.asyncio
async def test_create_scream_with_text(mock_message, mock_innoscream_api, sample_scream):
    ...
    with patch("src.bot.__main__.subscribers", {123, 456}):
        with patch("src.bot.__main__.settings") as mock_settings:
            ...
            mock_innoscream_api.create_scream.assert_called_once_with(123, "Test message")
            assert mock_message.bot.send_message.call_count == 2


@pytest.mark.asyncio
async def test_create_reaction(mock_callback_query, mock_innoscream_api, sample_scream):
    ...
    with patch("src.bot.__main__.subscribers", {123, 456}):
        with patch("src.bot.__main__.bot") as mock_bot:
            ...
            with patch("src.bot.__main__.asyncio.sleep", AsyncMock()):
                from src.bot.__main__ import send_daily_top_scream
                ...
                async def run_once():
                    ...
                    meme = await mock_innoscream_api.generate_meme(scream.scream_id)
                ...
                mock_innoscream_api.get_most_voted_scream.assert_called_once_with("day")
