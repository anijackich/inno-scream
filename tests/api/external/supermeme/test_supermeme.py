...
from src.api.external.supermeme.supermeme import (
    Supermeme,
    MemeTemplate,
    MemeTemplateProps,
    Caption,
...
async def test_search_meme_templates(sample_meme_template):
    ...
    mock_response.json = AsyncMock(
        return_value={'memeTemplates': [sample_meme_template.model_dump()]}
    )
    ...
    with patch(
        'src.api.external.supermeme.supermeme.AsyncClient',
        return_value=mock_client,
    ):
        with patch(
            'src.api.external.supermeme.supermeme.MemeTemplate.model_validate'
        ) as mock_validate:
...
async def test_get_meme_template_props(sample_meme_template, sample_next_data):
    ...
    mock_response.content = (
        ...
        + json.dumps(sample_next_data).encode()
        + b'</script></body></html>'
    )
    mock_response.raise_for_status = AsyncMock()
    ...
    with patch(
        ...
        with patch(
            'src.api.external.supermeme.supermeme.print'
        ):  # Suppress print statement
            supermeme = Supermeme()
            result = await supermeme.get_meme_template_props(
            ...
            mock_client.get.assert_called_once_with(
                f'/meme/{sample_meme_template.name}'
            )
            mock_response.raise_for_status.assert_called_once()
...
async def test_get_meme_template_props_invalid_schema():
    ...
    mock_response.content = (
        b'<html><body>'
        b'<script id=\'__NEXT_DATA__\'>{"props":{}}</script>'
        b'</body></html>'
    )
    mock_response.raise_for_status = AsyncMock()
    ...
    with patch(
        ...
        with patch(
            ...
            supermeme = Supermeme()
            with pytest.raises(
                ValueError, match='Invalid __NEXT_DATA__ schema'
            ):
                await supermeme.get_meme_template_props(MagicMock())
...
async def test_meme_template_props_model(sample_meme_template_props):
    ...
    assert (
        sample_meme_template_props.image_src == 'https://example.com/meme.jpg'
    )
    assert sample_meme_template_props.image_name == 'test_meme'
    assert (
        sample_meme_template_props.image_description == 'Test meme description'
    )
