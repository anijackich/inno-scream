...
from src.api.external.quickchart.quickchart import (
    QuickChart,
    Chart,
    ChartData,
    Dataset,
...
def sample_chart():
    return Chart(
        ...
        data=ChartData(
            ...
            datasets=[
                Dataset(
                    label='posts', data=[34, 79, 22], backgroundColor='black'
                )
            ],
        ),
...
async def test_chart(sample_chart):
    ...
    with patch(
        'src.api.external.quickchart.quickchart.AsyncClient',
...
async def test_chart_data_model():
    chart_data = ChartData(
        labels=['Mon', 'Tue', 'Wed'],
        datasets=[
            Dataset(label='posts', data=[34, 79, 22], backgroundColor='black')
        ],
    )
