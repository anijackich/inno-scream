...
from src.api.models import Scream
...
async def test_split_text_to_lines():
    ...
    assert len(lines) > 1
    assert all(service.get_text_sizes(line, font)[0] <= width for line in lines)
...
async def test_generate_meme(sample_scream, sample_meme_template_props):
    ...
    mock_supermeme.get_meme_template_props.return_value = sample_meme_template_props
    ...
    with patch("src.api.memes.service.get_scream", mock_get_scream):
        with patch("src.api.memes.service.Supermeme", return_value=mock_supermeme):
            with patch("src.api.memes.service.fetch_meme_image", mock_fetch_image):
                with patch("src.api.memes.service.insert_text_on_image", mock_insert_text):
                    with patch("src.api.memes.service.image2bytes", mock_image2bytes):
                        with patch("src.api.memes.service.ImageFont.truetype") as mock_font:
                            result = await service.generate_meme(AsyncMock(), 1)
                            
                            assert result == b"test_meme_data"
                            mock_get_scream.assert_called_once()
                            mock_supermeme.search_meme_templates.assert_called_once_with(sample_scream.text)
                            mock_supermeme.get_meme_template_props.assert_called_once_with("test_meme")
                            mock_fetch_image.assert_called_once_with(sample_meme_template_props)
                            mock_insert_text.assert_called_once()
                            mock_image2bytes.assert_called_once_with(mock_image)